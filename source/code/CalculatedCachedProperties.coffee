(urequire:
    rootExports: 'CalculatedCachedProperties')
...
class CalculatedCachedProperties
        ...
        CCP_allCalcProperties:
            get: ->
                if not @constructor::hasOwnProperty '_allCalcProperties' # use cached result, shared by all instances
                    Object.defineProperty @constructor::, '_allCalcProperties',
                        value: @CCP_getAllCalcProperties(), enumerable: false
    ...
    # To pass options from subclasses, the last constructor's argument `CCP_options` property carries them, else empty {}
    constructor: (args...)->
        @_CCP_defineCalcProperties args[args.length-1]?.CCP_options
    ...
    CCP_clean: (cleanArgs...)->
        # check _CCP_defineCalcProperties has been called (i.e super has been called in case of subclass)
        if not _.isFunction @constructor.isDebug
            @_CCP_defineCalcProperties()
